        ...
        switch (v.getId()){
            ...
            case R.id.dsyy_edit_yes:
              ...
              /*  String car_role="";
                ...
                Log.e("time","booktimestampSelected"+booktimestampSelected);
                if(type.equals("YCK")){
                    if(car_role.equals("1")){//标的车,有车牌
                        applyAccessHttpData(mUsermanager.getUserToken(),mUsermanager.getFrontRole(),selectTask.getAsString(TaskCK.id),licenseno_selected,car_role,
                                assessor_nameStr,assessor_mobileStr,assess_selected,booktimestampSelected,case_from_seleced,assess_selected_id,areaname_selected_id,selectTask.getAsString(TaskCK.vehicleBrand),group_id,OkCallbackManager.getInstance().applyAccessCallback(mContext,DSappointment.this));
...
//                pvTime.show();
    ...
    public void setTimeandTimeStamp(){
        appointtimestr=mSYear+"-"+(mSMonth+1)+"-"+mSDay;
        timestampSelected=String.valueOf(TimeUtil.timeStamp(appointtimestr,"yyyy-MM-dd HH:mm:ss"));
